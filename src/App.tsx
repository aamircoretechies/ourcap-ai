...
const SITE_NAME = "Employee Attendance & Leave Management"

type IconName = 'arrow' | 'check' | 'clock' | 'calendar' | 'people' | 'shield' | 'spark' | 'chart' | 'menu' | 'close'

const GOOGLE_SCRIPT_URL =
  "https://script.google.com/macros/s/AKfycbwwLxYf-0OoxZbBAdw3-RW_5mem-Hje0VaVwrFdoR0AavsxBRpgIQeWGaN2vaIocDHkHA/exec";
  ...
  return <div className="page-shell" id="top">
    ...
    <main id="main">
      <section className="section-wrap pb-20 pt-14 text-center sm:pt-20">
        <Reveal><span className="eyebrow"><Icon name="spark" className="h-4 w-4" /> Smart workforce operations</span>
          ...
          <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row"><button className="btn-primary" onClick={() => setEnquiryOpen(true)}>Enquiry Now <Icon name="arrow" className="h-4 w-4" /></button><a className="btn-secondary" href={PRODUCT_URL} target="_blank" rel="noreferrer">Explore the platform</a></div>
    ...
    {enquiryOpen && (
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
        <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" onClick={() => setEnquiryOpen(false)} aria-hidden="true" />
        <div className="relative w-full max-w-md overflow-hidden rounded-[24px] bg-white p-6 shadow-2xl sm:p-8">
          <div className="mb-6 flex items-center justify-between">
            <h2 className="text-2xl font-extrabold tracking-tight text-ink">Enquiry Now</h2>
            <button className="grid h-10 w-10 place-items-center rounded-full bg-slate-100 text-slate-500 hover:bg-slate-200 hover:text-ink transition-colors" onClick={() => { setEnquiryOpen(false); setSubmitStatus(null); }} aria-label="Close modal">
          ...
          <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <input type="text" id="name" required className="block w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-indigo focus:bg-white focus:outline-none focus:ring-4 focus:ring-violet/10 transition-all" placeholder="Your name" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />
            </div>
            <div>
              <label htmlFor="email" className="mb-1.5 block text-sm font-semibold text-slate-700">Email</label>
              <input type="email" id="email" required className="block w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-indigo focus:bg-white focus:outline-none focus:ring-4 focus:ring-violet/10 transition-all" placeholder="you@company.com" value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} />
            </div>
            <div>
              <label htmlFor="phone" className="mb-1.5 block text-sm font-semibold text-slate-700">Phone</label>
              <input type="tel" id="phone" required className="block w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-indigo focus:bg-white focus:outline-none focus:ring-4 focus:ring-violet/10 transition-all" placeholder="Your phone number" value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })} />
            </div>
            <div>
              <label htmlFor="message" className="mb-1.5 block text-sm font-semibold text-slate-700">Message</label>
              <textarea id="message" rows={3} required className="block w-full resize-none rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-indigo focus:bg-white focus:outline-none focus:ring-4 focus:ring-violet/10 transition-all" placeholder="How can we help?" value={formData.message} onChange={e => setFormData({ ...formData, message: e.target.value })}></textarea>
            </div>
            {submitStatus && (
              <div className={`rounded-xl p-4 text-sm font-semibold ${submitStatus.type === 'success' ? 'bg-emerald-50 text-emerald-600 border border-emerald-200' : 'bg-red-50 text-red-600 border border-red-200'}`}>
                {submitStatus.message}
              </div>
            )}
            <button type="submit" disabled={isSubmitting} className="btn-primary mt-2 w-full !min-h-[52px] disabled:cursor-not-allowed disabled:opacity-70">
